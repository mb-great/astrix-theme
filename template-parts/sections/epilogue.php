  ...
  <div style="position: relative; z-index: 2; background: linear-gradient(150deg, #2A2019, #17130F 60%); color: #F5F1EA; overflow: hidden;">
    ...
    <div class="grid-12" style="position: relative; z-index: 1; display: grid; grid-template-columns: repeat(12, 1fr); gap: 24px; padding: clamp(90px, 16vh, 170px) clamp(28px, 5vw, 72px);">
      <div data-reveal style="grid-column: 1 / span 7; display: flex; flex-direction: column; gap: clamp(28px, 5vh, 48px);">
        ...
        <p style="margin: 0; font-size: clamp(17px, 1.7vw, 22px); line-height: 1.7; color: rgba(245,241,234,0.72); max-width: 52ch;"><?php echo esc_html(astrix_field('epilogue_body', $front_id)); ?></p>
        <div style="display: flex; align-items: center; gap: 28px; flex-wrap: wrap; margin-top: 4px;">
          ...
          <a href="mailto:<?php echo esc_attr(astrix_setting('email')); ?>" class="ax-underline-light" style="display: inline-flex; align-items: center; gap: 10px; font-size: clamp(17px, 1.6vw, 21px); font-weight: 500; color: #F5F1EA; padding: 6px 0; border: none; border-bottom: 1px solid rgba(245,241,234,0.28); transition: border-color 0.3s ease;">
